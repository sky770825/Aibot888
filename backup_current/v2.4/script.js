...
const {
    ...
    'real-estate-html': `請幫我建立一個簡單的房仲個人網頁（HTML方案），包含以下功能： **專案需求：** - 目標用戶：房仲業務員和潛在客戶 - 核心功能：物件展示、客戶見證、專業介紹、聯絡預約 - 設計風格：專業、信任感、使用藍色和白色配色 - 技術要求：純HTML + CSS + JavaScript，響應式設計 **請依序實作：** 1. 單頁面HTML結構（導航、首頁、物件展示、關於我、聯絡） 2. 房仲專用CSS樣式（專業色彩、信任感設計） 3. 響應式設計（手機、平板、桌面適配） 4. 物件展示區域（圖片、基本資訊、價格） 5. 客戶見證區塊（推薦信、評分） 6. 專業介紹（經歷、證照、服務特色） 7. 聯絡預約表單（看屋預約、諮詢表單） 8. 基本互動功能（物件篩選、表單驗證） 請提供完整的HTML、CSS、JavaScript程式碼，確保可以直接在瀏覽器中運行。`,
