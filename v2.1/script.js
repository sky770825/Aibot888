...
請提供完整的程式碼實作和部署指南。`
    ...
    function generateCustomCommand(commandId) {
        ...
        // 根據選中的選項生成客製化指令
        let customTemplate = `請建立懸浮按鈕 (Floating Action Button)：\n`;
        
        // 根據選項添加功能
        let hasSelectedOptions = false;
        
        // 位置設定
        if (selectedOptions.includes('fixed-position')) {
            customTemplate += `- 固定在螢幕右下角\n`;
            hasSelectedOptions = true;
        }
        
        // 形狀設計
        if (selectedOptions.includes('circular-shape')) {
            customTemplate += `- 圓形設計\n`;
            hasSelectedOptions = true;
        }
        
        if (selectedOptions.includes('gradient-bg')) {
            customTemplate += `- 漸層背景\n`;
            hasSelectedOptions = true;
        }
        if (selectedOptions.includes('shadow-effect')) {
            customTemplate += `- 添加立體陰影效果\n`;
            hasSelectedOptions = true;
        }
        if (selectedOptions.includes('back-to-top')) {
            customTemplate += `- 點擊後回到頂部\n`;
            hasSelectedOptions = true;
        }
        if (selectedOptions.includes('speed-dial')) {
            customTemplate += `- 支援展開式子按鈕功能\n`;
            hasSelectedOptions = true;
        }
        if (selectedOptions.includes('backdrop')) {
            customTemplate += `- 展開時顯示半透明背景遮罩\n`;
            hasSelectedOptions = true;
        }
        if (selectedOptions.includes('hover-animation')) {
            customTemplate += `- 添加懸停動畫效果\n`;
            hasSelectedOptions = true;
        }
        if (selectedOptions.includes('responsive')) {
            customTemplate += `- 響應式設計，手機版調整位置\n`;
            hasSelectedOptions = true;
        }
        if (selectedOptions.includes('material-design')) {
            customTemplate += `- 遵循 Material Design 設計規範\n`;
            hasSelectedOptions = true;
        }
        ...
        if (!hasSelectedOptions) {
            customTemplate = `請建立懸浮按鈕 (Floating Action Button)：
